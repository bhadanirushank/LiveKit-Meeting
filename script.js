...
// Replace UUID.randomUUID().toString() with client.bootstrapDevice()
// and map the variable name to its accessToken variable
const deviceIdRegex = /val (deviceId[A-Za-z0-9_]*) = UUID\.randomUUID\(\)\.toString\(\)/g;
content = content.replace(deviceIdRegex, 'val ($1, ${1}_token) = client.bootstrapDevice()');
